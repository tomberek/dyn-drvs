...
# NixOS/nix's own `nix-cmd` component -- the seventh real component in
# a row (after `nix-util`, `nix-store`, `nix-fetchers`, `nix-expr`,
# `nix-flake`, `nix-main` -- examples 09-14), directly depending on
# ALL SIX of those (confirmed via `src/libcmd/package.nix`'s own
# `propagatedBuildInputs`), plus two new dependencies: `editline`
# (interactive repl line editing) and `lowdown` (markdown rendering
# for `--help` text, `enableMarkdown ? true` by default).
#
# No generators, no custom_targets beyond ordinary `dependency()`
# lookups -- confirmed via direct reading of `src/libcmd/meson.build`.
#
# Same three structural facts as examples 09-14 apply unchanged
