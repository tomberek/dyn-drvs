...
# NixOS/nix's own `nix-flake` component -- the fifth real component in
# a row (after `nix-util`, `nix-store`, `nix-fetchers`, `nix-expr` --
# examples 09-12), directly depending on all four (confirmed via
# `src/libflake/package.nix`'s own `propagatedBuildInputs`).
#
# No new dependencies, no generators, no custom_targets beyond ordinary
# meson `configure_file` header generation -- confirmed via direct
# reading of `src/libflake/meson.build`. `call-flake.nix` is `#embed`ed
# (`src/libflake/flake.cc`), already handled generically by the
# `ninja -t deps` carry-forward fix added for `nix-store`'s own
# `schema.sql` gap (see example 10's own header comment) -- no NEW fix
# needed here.
#
# Same three structural facts as examples 09-12 apply unchanged
...
#
# Run with:
#   try-it-out/run-nix.sh build --impure --no-link --print-out-paths -f try-it-out/examples/13-accelerate-nix-flake.nix
#
...

{
  ...
  dyndrvShim ? null,
  # Same pinned rev examples 09-12 use.
