...
#   }
#
# Verified two ways: the resulting `libfreetype.so` is actually loaded
# and run by a small C test program (`FT_Init_FreeType`/
# `FT_Library_Version`), and `patchOutput` (below) shows the SAME
# package, `.overrideAttrs`'d with one extra source-file patch,
# confirming `overrideAttrs` correctly reaches phase 1 and that only the
# patched file's own translation units register as new dynamic
# derivations, not the whole package. See
...
# nixpkgs' own real multi-output machinery runs there unmodified).
#
# Run with:
#   try-it-out/run-nix.sh build --no-link --print-out-paths -f try-it-out/examples/07-accelerate-real-package.nix
#
