...
# way `try-it-out/run-nix.sh` does for the example files, including
# passing `system` explicitly (sourced from `nix config show`, a plain
# config query, not an eval) so this needs no `--impure` for
